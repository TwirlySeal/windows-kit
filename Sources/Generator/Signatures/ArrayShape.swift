...
struct ArrayShape {
    ...
    init(parsing span: inout ParserSpan) throws {
        self.rank = try MetadataDB.parseCompressedUnsignedInteger(span: &span)
        guard rank >= 1 else {
            throw ArrayShapeError.invalidRank
        }
        
        let numSizes = try MetadataDB.parseCompressedUnsignedInteger(span: &span)
        self.sizes = try [UInt32](count: Int(numSizes)) {
            try MetadataDB.parseCompressedUnsignedInteger(span: &span)
        }
        
        let numLoBounds = try MetadataDB.parseCompressedUnsignedInteger(span: &span)
